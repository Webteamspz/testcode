convert.$(document).ready(function() {
    if (!convert.$('body').hasClass('sonno-test-v3')) {
        var cartIsempty = convert.$('#cart_item_count').attr('value');
        convert.$('body').addClass('sonno-test-v3');
        if(cartIsempty > 0){
        	loadTest();
        }
    }
    ...
    function loadTest() {
        ...
        var tpBanner = '<div class="tp-banner"><img class="tp-img" src="https://iili.io/HrgmzoN.png" alt="Trust Piolt"><div class="tp-contetnt"><h5 class="tp-rating">Rating score: <span>4.8/5.0</span></h5><a class="tp-review" href="javascript:void(0)">Based on <span class="bold-text">2K+ Reviews</span></a></div></div>';
        ...
        convert.$('.lineItemContainerOuter').each(function(index, ele) {
            ...
            convert.$('<div class="price-wrapper">You saved <span class="save-price">' + price + '</span></div>').insertAfter('.lineItem-' + index + ' .custom-wrap');
        ...
        convert.$('#mattfont').html('<label for="add-mattress">Add Mattress And Save <input id="add-mattress" type="checkbox"/></label>');
